...
environment = "stg"                                                    #  Update required
non_prd_env = "eunp"
country_environment = "eustg"                                          #  Update required
...
default_additional_tags = {
  ...
  "ohi:environment" = "eustg-euw1",                                                                   #  Update required
  "ohi:stack-name"  = "eustg-euw1-common-tf-initial-setup-pipeline"       #  Update required
  }
default_non_prd_additional_tags= {
  ...
  "ohi:stack-name"  = "eustg-euw1-common-tf-initial-setup-pipeline"  
  }
...
cloudfront_config = {
  "common-s3" = {
    ...
    acm_certificate_domain = "stg.eu.ohiomron.eu"                  #  Update required
    existing_domain_name  = "stg.eu.ohiomron.eu"                   #  Update required
    subdomain_name = "common-terms-cdn.stg.eu.ohiomron.eu"         #  Update required
...
iam_policy_config = {
  ...
  auth-policy7 = {
    ...
    iam_policy_template_path = "./../resource-policy-template/iam-policy-permission-template/common-auth-service-lambda-role/cognito.tpl"
     template_variable = {
      cognito="eu-west-1_qhQXdzMns"                                                           #  Update required
     }
     tag="auth_additional_tags"
    },
  be-policy1 = {
...
security_groups = [
  {
    ...
    egress_rules = [
      {
        description = "eustg-ofs-user-report"                         
        from_port   = 7071
        to_port     = 7071
        protocol    = "tcp"
        security_groups = []
        ipv6_cidr_blocks  = []
        prefix_list_ids   = []
        cidr_blocks = ["10.0.31.0/24"]
      },
       {
        description = "eustg-ofs-user-report"              #  Update required
        from_port   = 6061
        to_port     = 6061
        protocol    = "tcp"
        ipv6_cidr_blocks  = []
        prefix_list_ids   = []
        security_groups   = []                              #  Update required
        cidr_blocks = ["10.0.31.0/24"]
      },
      {
        description = "Allow outbound HTTP"
