...
exports.handler = async (event, context, callback) => {
	...
	try {
		...
		const table = await getAppConfig(body.app);
		const date = Date.now();

		if (body.measurements) await DB.saveReadings(body.app, body.userID, body.measurements, table, date);
		const response = {
			success: true,
