...
const test = async () => {
	describe(TEST_CONSTANTS.TITLE, function () {
		...
		it(`Test#${testCase++} for ${TEST_CONSTANTS.CODE_URI} with cognito error within the system`, async () => {
			...
			const expectedResponse = {
				success: false,
				errorCode: CONSTANTS.ERRORS.VERIFICATION_MAIL_FAILED.CODE,
				message: CONSTANTS.ERRORS.VERIFICATION_MAIL_FAILED.MESSAGE
			};
			await callIndex(params, expectedResponse);
		});
		...
		it(`Test#${testCase++} for ${TEST_CONSTANTS.CODE_URI} with cognito Errors within the system`, async () => {
			const params = JSON.parse(JSON.stringify(TEST_CONSTANTS.INPUTS.SUCCESS));
		...
		it(`Test#${testCase++} for ${TEST_CONSTANTS.CODE_URI} with cognito Errors within the system`, async () => {
			const params = JSON.parse(JSON.stringify(TEST_CONSTANTS.INPUTS.SUCCESS));
			params.body.emailAddress = 'invalidPrameter@user.com';
			const expectedResponse = {
				success: false,
				errorCode: CONSTANTS.ERRORS.VERIFICATION_MAIL_FAILED.CODE,
				message: CONSTANTS.ERRORS.VERIFICATION_MAIL_FAILED.MESSAGE
